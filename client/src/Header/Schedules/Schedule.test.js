...
describe('ScheduleList Component', () => {
  ...
  it('should render a schedule', () => {
    ...
    const { queryByTestId, queryByText } = render(<ScheduleList schedules={[testSchedule]} />);
  ...
  it('should render a schedule', () => {
    const testScheduleOne = {
      ...
      start_time: moment().startOf('h').toISOString(),
    };
    ...
    const testScheduleTwo = {
      ...
      start_time: moment().startOf('h').add(3, 'h').toISOString(),
    };

    const { queryByTestId, queryByText, debug } = render(<ScheduleList schedules={[testScheduleOne, testScheduleTwo]} />);
